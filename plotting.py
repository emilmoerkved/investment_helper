...
class Plotting:
    ...
    def __init__(self):
        ...
        self.font_title = {'family': 'Courier New',
                           'color': '#822304',
                           'weight': 'heavy',
                           'size': 25,
                           }
        self.font_xylabel = {'family': 'Courier New',
                             'color': '#8787a1',
                             'weight': 'light',
                             'size': 16,
                             }
        self.font_cursor = {'family': 'Courier New',
                            'style': 'italic',
                            'color': '#303d34',
                            'weight': 'bold',
                            'size': 15,
    ...
    def _create_normal_plot(self):
        # user_req dict with keys: ticker, period, mva_50, mva_200, support, resistance
        self._clear_figure()
    ...
    def _plot_data_normal(self):
        self._axlist[0].plot_date(self._df['Close'].index, self._df['Close']._values, '-', linewidth=2)
        self._axlist[1].plot_date(self._df['Volume'].index, self._df['Volume']._values, '-', linewidth=0.8)
    ...
    def _customize_axes_normal(self):
        self._axlist[0].set_title(self._user_input.get_title(), pad=45, fontdict=self.font_title)
        self._axlist[0].set_ylabel('Close Price [$]', fontdict=self.font_xylabel)
        self._axlist[0].yaxis.set_label_coords(-0.1, 0.5)
        self._axlist[1].set_ylabel('Volume', fontdict=self.font_xylabel)
        self._axlist[1].yaxis.set_label_coords(-0.1, 0.5)
        self._axlist[1].yaxis.set_major_formatter(mticker.FormatStrFormatter('%d'))
        plt.setp(self._axlist[0].get_xticklabels(), visible=False)
    ...
    def _get_mva_df(self, mva):
        if self._user_input.get_period() == 'max':
            ...
            # start_date is today - period - mva_len
            start_date = get_date_from_period(self._user_input.get_period(), mva)
        df_mva = self._financial_data_object.get_stock_df_by_dates(start_date, datetime.date.today())
        return df_mva['Close']

    def _plot_mva(self):
        mva_list = self._get_mva_list()
        for mva in mva_list:
            self._axlist[0].plot(self._get_mva_df(mva).rolling(window=mva).mean(),
                                     label=self._create_mva_label(mva),
                                     linewidth=0.8)
        self._axlist[0].legend(loc='best', fontsize='small')

    def _set_df(self, df):
        self._df = df

    def _get_mva_label(self):
        return str(self._get_mva_len()) + ' days moving average'
    ...
    def _plot_tech_analysis_candlestick(self):
        ...
        if self._user_input.is_resistance_tech_analysis_requested():
            ...
            resistance_df = self._financial_data_object.technical_analysis.get_closest_resistance_levels()
            for date in resistance_df.index:
                ind_start = df_indexed.loc[df_indexed['Date'] == date].index._values[-1]
    ...
    def _customize_axes_candlestick(self, legndtxt_mva_list):
        ...
        self._axlist[0].set_title(self._user_input.get_title(), pad=45, fontdict=self.font_title)
        self._axlist[0].set_ylabel('Open, High, Low, Close', fontdict=self.font_xylabel)
        ...
        self._axlist[2].set_ylabel('Volume', fontdict=self.font_xylabel)
    ...
    def _create_candlestick_plot(self):
        ...
        mva_list, legndtxt_mva_list = ([] for i in range(2))
    ...
    def create_cursor_value(self, coord):
        self._axlist[0].texts = []  # To clear earlier texts from self.ax1.texts
        if coord.inaxes:
            self._update_scatter(coord)
            self._update_cursor(coord)
            coord.canvas.draw()  # coord has canvas (FigureCanvasTkAgg) as attribute
            self._clear_scatter()
    ...
    def _update_cursor(self, coord):
        self._axlist[0].text(x=0.3, y=1.05, s=self._get_cursor_text(coord), fontdict=self.font_cursor,
